...
public class Discretization {
    ...
    public static boolean isNumeric(Variable variable){
    	return isNumeric(variable,null);
    }
     ...
     * 
     * @param variable <code>Variable</code>
     * @param numericParam if not null it will store the max, min ,mean and standard deviation 
     * @return true if the variable is numeric
     */
    public static boolean isNumeric(Variable variable,java.util.DoubleSummaryStatistics dataStatistic){
        ...
        for (int i = 0; i < states.length; i++)
        {
            try
            {
                ...
                {
                    double value = Double.parseDouble (states[i].getName ());
                    if (dataStatistic != null)
                    	dataStatistic.accept(value);                    
                }else
                {
